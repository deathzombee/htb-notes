...
```shell-session
cdeez@htb[/htb]$ nslookup -type=NS zonetransfer.me
...
```shell-session
cdeez@htb[/htb]$ nslookup -type=any -query=AXFR zonetransfer.me nsztm1.digi.ninja
...
cdeez@htb[/htb]$ export NS="d.ns.facebook.com"
cdeez@htb[/htb]$ export WORDLIST="numbers.txt"
cdeez@htb[/htb]$ gobuster dns -q -r "${NS}" -d "${TARGET}" -w "${WORDLIST}" -p ./patterns.txt -o "gobuster_${TARGET}.txt"
